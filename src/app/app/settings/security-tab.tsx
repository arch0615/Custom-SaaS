...
function PasswordInput({
  ...
  return (
    <div className="relative">
      <Key className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" />
      <input
        id={id}
        name={name}
        type={show ? "text" : "password"}
        placeholder={placeholder}
        className="h-11 w-full rounded-lg border border-slate-200 bg-slate-50/50 pl-10 pr-10 text-sm text-slate-700 placeholder:text-slate-400 focus:border-primary focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary/15"
      />
      <button
        type="button"
        onClick={() => setShow((v) => !v)}
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form action={formAction} className="space-y-4">
          <div>
            <label htmlFor="current" className="mb-1.5 block text-xs font-medium text-slate-700">
              Senha atual
            </label>
          ...
          <div>
            <label htmlFor="next" className="mb-1.5 block text-xs font-medium text-slate-700">
              Nova senha
            </label>
          ...
          <div className="flex flex-col items-start justify-between gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center">
            <p className="text-xs text-slate-500">Use uma senha forte com letras, números e símbolos</p>
            <button
              type="submit"
              disabled={pending}
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              <h2 className="text-sm font-semibold text-slate-900">Autenticação de dois fatores</h2>
              <p className="text-xs text-slate-500">
          ...
          <button
            ...
            disabled
            title="Em breve"
            className="inline-flex shrink-0 items-center gap-2 self-start rounded-lg border border-slate-200 bg-white px-4 py-2 text-xs font-medium text-slate-500 disabled:cursor-not-allowed"
          >
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <header className="mb-4">
          ...
          <p className="text-xs text-slate-500">Onde você está conectado agora.</p>
        </header>
        <div className="flex items-start justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50/40 p-4">
          <div className="flex items-start gap-3">
            <span className="inline-flex size-9 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700">
              <Monitor className="size-4" />
            </span>
            <div>
              <p className="text-sm font-semibold text-slate-900">Esta sessão</p>
              <p className="text-xs text-slate-500">{currentSessionAgent}</p>
            </div>
          </div>
